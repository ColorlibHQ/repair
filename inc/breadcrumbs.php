...
if ( ! function_exists( 'repair_breadcrumbs' ) ) {
    function repair_breadcrumbs( $args = array() ) {
        if ( is_front_page() ) {
            return;
        }
        ...
        $defaults  = array(
            ...
            'home_title'          => esc_html__( 'Home', 'repair' )
        );
        $args      = apply_filters( 'repair_breadcrumbs_args', wp_parse_args( $args, $defaults ) );
        ...
        */
        ...
        $html .= '<a class="bread-link s-text16 bread-home" href="' . esc_url( get_home_url('/') ) . '" title="' . esc_attr( $args['home_title'] ) . '">'.esc_html__( 'Home', 'repair' ).' <span class="lnr lnr-arrow-right"></span></a>';
        // Post
        if ( is_singular( 'post' ) ) {
            ...
            $html .= '<span class="s-text17 bread-search">'.esc_html__('Search results for:','repair') . get_search_query() . '</span>';
        } elseif ( is_404() ) {
            $html .= '<span>' . esc_html__( 'Error 404', 'repair' ) . '</span>';
        } elseif ( is_home() ) {
            $html .= '<span class="s-text17">' . get_the_title( get_option( 'page_for_posts' ) ) . '</span>';
        }
        $html .= '</div>';
        $html = apply_filters( 'repair_breadcrumbs_filter', $html );
