...
// enqueue css
function repair_common_custom_css() {

	wp_enqueue_style( 'repair-common', get_template_directory_uri().'/assets/css/common.css' );

	$headerBg          =  ! empty( get_header_image() ) ? 'url(' . esc_url( get_header_image() ) . ')' : '';
	$headerTextColor   = esc_attr( repair_opt( 'repair_headertextcolor', '#fff' ) );
	$headerbgcolor     = esc_attr( repair_opt( 'repair_headerbgcolor' ) );
	$headerOverlay     = esc_attr( repair_opt( 'repair_headeroverlaycolor' ) );
	$navbarbg 		   = esc_attr( repair_opt( 'repair_header_navbar_bgColor' ) );
	$stickynavbarbg    = esc_attr( repair_opt( 'repair_header_navbarsticky_bgColor' ) );
	$navmenuColor 			= esc_attr( repair_opt( 'repair_header_navbar_menuColor' ) );
	$navmenuHovColor 		= esc_attr( repair_opt( 'repair_header_navbar_menuHovColor', '#222' ) );
	$stickynavmenuColor 	= esc_attr( repair_opt( 'repair_header_sticky_navbar_menuColor' ) );
	$stickynavmenuHovColor 	= esc_attr( repair_opt( 'repair_header_sticky_navbar_menuHovColor' ) );
	$foftext1     	   = esc_attr( repair_opt( 'repair_fof_textonecolor_settings' ) );
	$foftext2     	   = esc_attr( repair_opt( 'repair_fof_texttwocolor_settings' ) );
	$fofbgcolor        = esc_attr( repair_opt( 'repair_fof_bgcolor_settings' ) );
	$footerbgImg       = repair_opt( 'repair_footer_bgimg_settings' );
	...
	$footerbgColor     = esc_attr( repair_opt( 'repair_footer_widget_bgColor_settings' ) );
	$footerTextColor   = esc_attr( repair_opt( 'repair_footer_widget_color_settings' ) );
	$anchorcolor 	   = esc_attr( repair_opt( 'repair_footer_widget_anchorcolor_settings' ) );
	$anchorhovcolor    = esc_attr( repair_opt( 'repair_footer_widget_anchorhovcolor_settings' ) );
	$widgettitlecolor  = esc_attr( repair_opt( 'repair_footer_widgettitlecolor_settings' ) );
	$themecolor  	   = esc_attr( repair_opt( 'repair_themecolor' ) );

	$footerbtombg  	   			= esc_attr( repair_opt( 'repair_footer_bottom_bgcolor_settings' ) );
	$footerbtomtextcolor 		= esc_attr( repair_opt( 'repair_footer_bottom_textcolor_settings' ) );
	$footerbtomanchorcolor 		= esc_attr( repair_opt( 'repair_footer_bottom_anchorcolor_settings' ) );
	$footerbtomanchorhovercolor = esc_attr( repair_opt( 'repair_footer_bottom_anchorhovercolor_settings' ) );
	...
	wp_add_inline_style( 'repair-common', $customcss );

}
add_action( 'wp_enqueue_scripts', 'repair_common_custom_css', 50 );