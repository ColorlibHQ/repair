...
add_action( 'wp_enqueue_scripts', 'repair_sl_enqueue_scripts' );
function repair_sl_enqueue_scripts() {

	wp_enqueue_script( 'repair-post-likes',REPAIR_COMPANION_DIR_URL . 'js/post-likes.js', array( 'jquery' ), '0.5', false );

	wp_localize_script( 'repair-post-likes', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'unlike'  => esc_html__( 'Unlike', 'repair' )
	) );
}
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_repair_process_simple_like', 'repair_process_simple_like' );
add_action( 'wp_ajax_repair_process_simple_like', 'repair_process_simple_like' );
function repair_process_simple_like() {
	// Repair
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'repair' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	$output = '';
	$nonce = wp_create_nonce( 'simple-likes-nonce' ); // Repair
	if ( $is_comment == 1 ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'repair' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'repair' );
		$icon = $icon_empty;
	}
...
// User Profile List
add_action( 'show_user_profile', 'repair_show_user_likes' );
add_action( 'edit_user_profile', 'repair_show_user_likes' );
function repair_show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'repair' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'repair' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
